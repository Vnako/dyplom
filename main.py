...
def deserialize_game_state(state):
    ...
    total_time_seconds = state.get("total_time_seconds", 0)
...
def render_saves_menu(screen, menu_font, setings_menu_image, pause_menu_buttons, slot_count=SAVE_SLOTS, selected_slot=None):
    ...
    if selected_slot is not None and globals().get("showing_level", False) and not is_cave:
        save_btn_pos = (wide_x + int(wide_width * 0.25), bottom_y)
    ...
    if selected_slot is not None and slot_has_save.get(selected_slot, False):
        load_btn_pos = (wide_x + int(wide_width * 0.50), bottom_y)
    ...
    # Back
    back_btn_pos = (wide_x + int(wide_width * 0.75), bottom_y)
...
def render_game_completed(screen, title_font, menu_font):
    """Render the game completed screen with stats, a restart and continue button."""
    # --- Додаємо дефолтні значення для статистики ---
    global total_time_seconds, opened_chests, defeated_enemies
    # Якщо змінні не визначені, задаємо дефолтні значення
    try:
        tts = total_time_seconds
    except Exception:
        tts = 0
    try:
        chests_count = len(opened_chests)
    except Exception:
        chests_count = 0
    try:
        enemies_count = len(defeated_enemies)
    except Exception:
        enemies_count = 0
    ...
    screen.blit(overlay, (0, 0))
    # Text
    text = title_font.render("Дякуємо за гру!", True, (255, 80, 80))
    text_rect = text.get_rect(center=(SCREEN_WIDTH // 2, SCREEN_HEIGHT // 2 - 250))
    screen.blit(text, text_rect)
    info_lines = [
        "",
        "Ви пройшли останній поверх підземелля.",
        "Тепер Ви можете почати спочатку або продовжити дослідження гри.",
        "Ваша статистика:",
        ""
    ]
    line_spacing = 50  # Збільшений міжстрочний інтервал
    for i, line in enumerate(info_lines):
        info_text = menu_font.render(line, True, (255, 255, 255))
        info_rect = info_text.get_rect(center=(SCREEN_WIDTH // 2, SCREEN_HEIGHT // 2 - 175 + i * line_spacing))
    ...
    stats_lines = [
        f"Час у грі: {time_str}",
        f"Відкрито скринь: {chests_count}",
        f"Вбито ворогів: {enemies_count}"
    ]
    stats_spacing = 50  # Збільшений міжстрочний інтервал для статистики
    for i, stat in enumerate(stats_lines):
        stat_text = menu_font.render(stat, True, (200, 220, 255))
        stat_rect = stat_text.get_rect(center=(SCREEN_WIDTH // 2, SCREEN_HEIGHT // 2 + 40 + i * stats_spacing))
        screen.blit(stat_text, stat_rect)
    # --- Горизонтальне розташування кнопок ---
    btn_w, btn_h = pause_menu_buttons.get_width(), pause_menu_buttons.get_height()
    spacing = 40  # Відстань між кнопками
    total_width = btn_w * 2 + spacing
    start_x = (SCREEN_WIDTH - total_width) // 2
    btn_y = SCREEN_HEIGHT // 2 + 200
    # Кнопка "Почати спочатку"
    restart_rect = pygame.Rect(start_x, btn_y, btn_w, btn_h)
    screen.blit(pause_menu_buttons, (start_x, btn_y))
    restart_text = menu_font.render("Почати спочатку", True, (255, 255, 255))
    restart_text_rect = restart_text.get_rect(center=restart_rect.center)
    screen.blit(restart_text, restart_text_rect)
    # Друга кнопка "Продовжити гру"
    btn2_x = start_x + btn_w + spacing
    continue_rect = pygame.Rect(btn2_x, btn_y, btn_w, btn_h)
    screen.blit(pause_menu_buttons, (btn2_x, btn_y))
    continue_text = menu_font.render("Продовжити гру", True, (255, 255, 255))
    continue_text_rect = continue_text.get_rect(center=continue_rect.center)
    screen.blit(continue_text, continue_text_rect)
    return restart_rect, continue_rect
...
while running:
    ...
    # --- Додаємо підрахунок часу у грі ---
    if showing_level:
        # --- Музика для печери ---
        is_cave = "cave" in str(level_path).lower() or (level_path.name.startswith("level") and level_path.name != "level0.lvl")
        if is_cave:
            if current_music != "menu12":
                stop_music()
                try:
                    music_file = str(ASSETS_DIR / "audio" / "menu12.wav")
                    play_music(music_file, loop=True)
                    current_music = "menu12"
                except FileNotFoundError:
                    print("Файл menu12.wav не знайдено!")
        elif level_path.name == "level0.lvl":
            if current_music != "start_loc_bg2":
                stop_music()
                try:
                    music_file = str(ASSETS_DIR / "audio" / "start_loc_bg2.wav")
                    play_music(music_file, loop=True)
                    current_music = "start_loc_bg2"
                except FileNotFoundError:
                    print("Файл start_loc_bg2.wav не знайдено!")
        now = pygame.time.get_ticks()
        # Додаємо секунди, якщо пройшла хоча б 1 секунда
        if now - last_time_update >= 1000:
            total_time_seconds += (now - last_time_update) // 1000
            last_time_update = now
    # --- Фінальний екран після 10 рівня ---
    if showing_game_completed:
        screen.blit(mainmenu_bg, (0, 0))
        restart_rect, continue_rect = render_game_completed(screen, title_font, menu_font)
        cursor.draw(screen)
        pygame.display.flip()
        for event in events:
            if event.type == pygame.QUIT:
                ...
                if restart_rect.collidepoint(event.pos):
                    ...
                    upgraded_statues.clear()
                    level_select_shown = False
                    current_level_number = 1
                    game_completed_once = False 
                    break
                elif continue_rect.collidepoint(event.pos):
                    # Продовжити гру: повертаємо гравця на стартову локацію (level0.lvl)
                    showing_game_completed = False
                    showing_level = True
                    showing_menu = False
                    boss_defeated_and_portal_used = False
                    # --- Додаємо: скидаємо стан і завантажуємо стартову локацію ---
                    level_path = LEVELS_DIR / "level0.lvl"
                    level_data = parse_level_file(level_path)
                    player = None
                    camera = None
                    ...
                    showing_saves = False
                    selected_save_slot = None
                    # --- Додаємо: встановлюємо current_level_number на наступний рівень ---
                    current_level_number += 1
                    # --- Додаємо: прапорець, що це вже не перше проходження ---
                        ...
                        if rect.collidepoint(mouse_pos):
                            ...
                            if text == "Вихід":
                                running = False
                                 ...
                                 # --- Додаємо відкриття user.html, якщо підказки увімкнені ---
                                if settings.get("hints", True):
                                    webbrowser.open("user.html")
                                showing_menu = False
                                showing_level = True
        ...
        for item in items:
            if item.current_frame == 1:
                if current_time - item.clicked_time >= 500:
                    ...
                    opened_chests.add((item.rect.x, item.rect.y))
                ...
                if npc.type == 'enter' and not level_transitioning:
                    if settings.get("level_select", False):
                        ...
                        if boss_defeated_and_portal_used:
                            # --- Додаємо: якщо це фінальний рівень, показати фінальний екран ---
                            if level_path.name == "level10.lvl":  # або "level10.lvl" для справжньої гри
                                if not game_completed_once:
                                    showing_level = False
                                    showing_game_completed = True
                                    boss_defeated_and_portal_used = False
                                    game_completed_once = True
                                # boss_defeated_and_portal_used = False  # <-- НЕ скидаємо тут!
                                break  # Вийти з обробки NPC, щоб не було переходу на наступний рівень
                            # Інакше — стандартний перехід на наступний рівень
                    ...
                    save_statues_state(statues)
                    level_transitioning = True
                    if level_path.name == "level10.lvl":
                        # Це фінальний рівень, показуємо фінальний екран
                        boss_defeated_and_portal_used = False  # Скидаємо одразу, щоб не зациклити
                        showing_level = False
                        showing_game_completed = True
                        level_transitioning = False
                        break
