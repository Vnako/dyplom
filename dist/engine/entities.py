...
class Player:
    ...
    def handle_input(self, active_keys):
        ...
        # Підтримка WASD та стрілок
        left = pygame.K_a in active_keys or pygame.K_LEFT in active_keys
        right = pygame.K_d in active_keys or pygame.K_RIGHT in active_keys
        up = pygame.K_w in active_keys or pygame.K_UP in active_keys
        down = pygame.K_s in active_keys or pygame.K_DOWN in active_keys
...
class Enemy:
    ...
    def handle_movement(self, blocks, statues, items=None):
        ...
        for block in blocks:
            if block.is_solid and self.hitbox.colliderect(block.rect):
                self.rect.x = initial_position[0]
                self.hitbox.x = initial_hitbox[0]
                break
        # Перевірка колізій із предметами (items) по осі X
        for item in items:
            if item.is_solid and self.hitbox.colliderect(item.rect):
                self.rect.x = initial_position[0]
                self.hitbox.x = initial_hitbox[0]
                break
        # Перевірка колізій із статуями по осі X
        for statue in statues:
            if statue.is_solid and self.hitbox.colliderect(statue.rect):
                self.rect.x = initial_position[0]
                self.hitbox.x = initial_hitbox[0]
                break
        ...
        for block in blocks:
            if block.is_solid and self.hitbox.colliderect(block.rect):
                ...
                self.hitbox.y = initial_hitbox[1]
                break
        # Перевірка колізій із предметами (items) по осі Y
        for item in items:
            if item.is_solid and self.hitbox.colliderect(item.rect):
                self.rect.y = initial_position[1]
                self.hitbox.y = initial_hitbox[1]
